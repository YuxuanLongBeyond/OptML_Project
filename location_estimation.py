...
def define_parameters(W, V, num, edge_num):
    """
    Prepare all the parameters necessary for the optimization
    """
...
def compute_all_l2(Pc, y):
    """
    Compute L2 norm piecewise in the objective
    """
    Pc_y = np.dot(Pc, y)
...
def ls_solver(L, parameters, num):
    """
    Implement least squares with squared L2 loss
    """
...
def admm_solver(parameters, y0, lam0, num, edge_num, max_iter, l1_prox, delta, tau):
    """
    Solve the optimization problem by scaled-ADMM
    """
    Q = parameters['Q']
    P = parameters['P']
    Pc = parameters['Pc']
...
def irls_solver(parameters, num, edge_num, max_iter, delta, use_l1):
    """
    Solve the optimization problem by IRLS
    """
    Pc = parameters['Pc']
    Q = parameters['Q']
    w = 1
...
def main(noise_variance, W, V, show_plot):
    """
    Run LS, ADMM, IRLS to compute the camera locations
    Compare the results with the ground-truth locations
    """
    num = W.shape[0] # number of cameras
    edge_num = V.shape[0] # number of edges (i.e. connections)
...
#    V = (V.T / np.sqrt(np.sum(V ** 2, axis = 1))).T
    ...
    # run ADMM
    l1_prox = True # default to L1 proximal operator; otherwise, L2 proximal operator
    ...
    max_iter = 100 # iteration number
    delta = 0.00001
    use_l1 = False # if False, use L2 as objective; otherwise, use L1 as objective
    ...
    # compute mean absolute error between ground truth and recovered locations
